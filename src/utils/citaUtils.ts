...
async function text2Iso(text: string): Promise<string> {
  ...
  const service = new aiServices(config.apiKey);
  const prompt = `La fecha y hora actual en Madrid es: ${currentDate.toISO()}. Te voy a dar un texto. 
  Necesito que ese texto extraigas la fecha y la hora del texto que te voy a dar y respondas con las misma en formato ISO.
  Me tienes que responder EXCLUSIVAMENTE con esa fecha y horarios en formato ISO, usando el horario 00:39 en caso de que no este especificada la hora.
  IMPORTANTE: La hora debe ser interpretada en horario de Madrid (Europe/Madrid).
  Por ejemplo, el texto puede ser algo como "el jueves 30 de mayo a las 12hs". En ese caso tu respuesta tiene que ser "2024-06-30T12:00:00.000+02:00"
  Por ejemplo, el texto puede ser algo como "Este viernes 31". En ese caso tu respuesta tiene que ser "2024-06-31T00:39:00.000+02:00"
